...
import (
	...
	"github.com/taskcluster/taskcluster/v30/workers/generic-worker/livelog"
...
func (l *LiveLogTask) Stop(err *ExecutionErrors) {
	...
	logURL := tcurls.API(config.RootURL, "queue", "v1", fmt.Sprintf("task/%v/runs/%v/artifacts/%v", l.task.TaskID, l.task.RunID, logName))
